 ...
 * @LastEditTime: 2024-09-30 17:14:42
 * @FilePath: /flatbed/utils/index.ts
 * @Description: utils
 */
...
const utils = {
  ...
  async uploadFile(url: string, option: UploadFileOption = {}): Promise<unknown> {
    ...
    if (beforeSelect) beforeSelect(params)
    ...
    if (params) Object.keys(params).forEach(k => fd.append(k, params[k]))
  ...
  removeEmpty<T extends object>(obj: T): Partial<T> {
    return Object.entries(obj)
      ...
      .reduce(
        (pre, [k, v]) => ({
          ...pre,
          [k]: (v = Object.prototype.toString.call(v) === '[object Object]' ? this.removeEmpty(v) : v),
        }),
        {},
      )
  },
